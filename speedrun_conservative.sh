...
if [ "$NUM_GPUS" -gt 1 ]; then
    ...
    TORCHRUN_CMD="python"
    echo "Using single GPU training with ULTRA conservative memory settings"
    # 超保守的批次大小设置
    BATCH_SIZE_ARGS="--device_batch_size=4 --max_seq_len=1024 --eval_tokens=1024"
else
    TORCHRUN_CMD="python"
...
# 基础模型训练 - 使用超保守参数
echo "Starting base model training with ULTRA conservative memory settings..."
echo "This will take much longer but should avoid OOM errors"
if [ "$NUM_GPUS" -gt 1 ]; then
